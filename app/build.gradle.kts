...
tasks.register("generateAppIcons") {
    ...
    doLast {
        val srcFile = file("$rootDir/app/logo_source.png")
        if (!srcFile.exists()) throw GradleException("logo_source.png not found at ${srcFile.absolutePath}")
        val img = ImageIO.read(srcFile) ?: throw GradleException("Could not decode logo_source.png")
        ...
        densities.forEach { (density, sizes) ->
            ...
            val dir = file("$projectDir/src/main/res/mipmap-$density")
            dir.mkdirs()
        ...
        val check = file("$projectDir/src/main/res/mipmap-xxxhdpi/ic_launcher.png")
        if (check.length() == 0L) throw GradleException("Icon generation ran but output is still empty")
        println("App icons regenerated (${check.length()} bytes for xxxhdpi ic_launcher.png)")
    }
}
